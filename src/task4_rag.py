...
DEFAULT_NO_ANSWER = "Я не знаю."
DEFAULT_NO_SOURCES = "нет релевантных фрагментов"
...
Если в контексте недостаточно информации, ответь: "Я не знаю."
...
def parse_model_response(
    ...
    if not steps:
        steps = build_default_steps(selected_hits)
    if not answer_text:
...
def build_default_steps(selected_hits: list[RetrievedHit]) -> list[str]:
    lead_hit = selected_hits[0]
    steps = [
        "Выполнил поиск по базе знаний и отобрал самые релевантные чанки.",
        f"Опёрся на фрагмент {lead_hit.title} из раздела {lead_hit.section}.",
